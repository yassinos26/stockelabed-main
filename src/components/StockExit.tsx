...
type StockExitProps = {
  products: Product[];
  onAddMovement: (productId: string, type: 'exit', quantity: number, note?: string) => void;
  onBack: () => void;
};

export function StockExit({ products, onAddMovement, onBack }: StockExitProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (selectedProduct && quantity > 0) {
      ...
      onAddMovement(selectedProduct.id, 'exit', quantity, note);
      setQuantity(1);
      setNote('');
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="text-white">Créer Sortie de Stock</h2>
      </div>
              ...
              {filteredProducts.map(product => (
                ...
                >
                  ...
                  <div className="text-right">
                    ...
                    {product.quantity === 0 && (
                      <p className="text-sm text-red-400">Rupture</p>
                    )}
                  </div>
                </button>
              ))}
        ...
        {selectedProduct && (
          <div className="bg-gray-800 rounded-lg p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-white mb-2">Quantité à retirer *</label>
                <input
                  type="number"
                  value={quantity}
                  onChange={(e) => {
                ...
                <p className="text-sm text-gray-400 mt-2">
                  Nouveau stock: {Math.max(0, selectedProduct.quantity - quantity)}
                </p>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Client, vente, perte, etc."
                  rows={3}
                  className="w-full bg-gray-700 text-white px-4 py-3 rounded-lg border border-gray-600 focus:border-[#1a8754] focus:outline-none"
                />
              </div>
              ...
              <button
                ...
                className="w-full bg-[#1a8754] text-white px-6 py-4 rounded-lg hover:bg-[#15693f] transition-colors inline-flex items-center justify-center gap-2 disabled:bg-gray-600 disabled:cursor-not-allowed"
              >
